...
structure CFunction =
   struct
      ...
      (* CHECK; thread as objptr *)
      val sporkForkThreadAndSetData = fn ty =>
...
fun convert (program as S.Program.T {functions, globals, main, ...},
   ...
   let
      ...
      val {get = sporkDataArg: Label.t -> Var.t * S.Type.t,
           set = setSporkDataArg, ...} =
         Property.getSetOnce (Label.plist,
                              Property.initRaise ("sporkDataArg", Label.layout))
      val () =
         List.foreach
         (functions, fn f =>
          let
                       ...
                       case stmt of
                          S.Statement.Bind {var, ty,
                             ...
                             let
                                val l =
                                   case l of
                                      NONE => Error.bug "Ssa2ToRssa.setSporkDataArg: Spork_getData not dominated by Spork"
                                    | SOME l => l
                                val var =
                                   case var of
                                      NONE => Var.newNoname ()
      ...
      val handlesSignals =
          ...
          * the requirements of synchronization
          * are a * subset of the requirements for signal handling
          *)
         true orelse
         S.Program.hasPrim
         (program, fn p =>
                                    ...
                                    (case toRtype ty of
                                     ...
                                     | SOME rty =>
                                          let
                                             ...
                                             val theCAS =
                                               PrimApp {dst = dst (),
                                                        prim = Prim.Ref_cas (SOME (Type.toCType rty)),
                                                        args = args'}
                                          in
                                    ...
                                    (case toRtype ty of
                                     ...
                                     | SOME rty =>
                                          let
                                             ...
                                             val theCAS =
                                               PrimApp {dst = dst (),
                                                        prim = Prim.Array_cas (SOME (Type.toCType rty)),
                                                        args = args'}
                                          in
                                            if not (Type.isObjptr rty) then
                                              add theCAS
